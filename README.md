...
2. Clone the repository
   ```
   git clone --branch master --recursive https://github.com/monero-classic-lab/Monero-Classic-V4-Wallet.git
   ```
   \* `master` - replace with the desired version tag (e.g. `v0.18.2.2`) to build the release binaries.
3. Prepare build environment
   ```
   cd Monero-Classic-V4-Wallet
...
4. Build
   ...
   ```
   \* `$(pwd)` - make sure you are in the `Monero-Classic-V4-Wallet` directory before running this command  
   \* `4` - number of CPU threads to use
5. Monero GUI Windows static binaries will be placed in  `Monero-Classic-V4-Wallet/build/x86_64-w64-mingw32/release/bin` directory
